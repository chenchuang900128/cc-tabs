	...
	<view class="cc-tabs" :class="{'tabs-fixed': fixed}" :style="{height: tabHeightVal, top:topFixed, 'margin-top':topMargin}">
		<scroll-view v-if="tabs.length" :id="viewId" :scroll-left="scrollLeft" scroll-x scroll-with-animation :scroll-animation-duration="240">
			<view class="tabs-item" :class="{'tabs-flex':!isScroll, 'tabs-scroll':isScroll}">
				<!-- tab -->
				<view class="tab-item" :style="{height: tabHeightVal, marginLeft:spaceLeft + 'px', 'line-height':tabHeightVal}" v-for="(tab, i) in tabs" :class="{'active': value===i}" :key="i" @click="tabClick(i)">
					{{getTabName(tab)}}
				</view>
...
<style lang="scss">
	.cc-tabs{
		...
		.tabs-line{
			z-index: 1;
			position: absolute;
			bottom: 24rpx; // 至少与.tabs-item的padding-bottom一致,才能保证在底部边缘
			width: 48rpx;
			height: 2px;
